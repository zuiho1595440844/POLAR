...
from tensorflow.keras.layers import Input, Conv2D, ZeroPadding2D, MaxPooling2D, Flatten, Dense, Dropout, Lambda
...
def dueling_dqn_model(input_shape, l2_regularizer=0.001):
    ...
    # Dueling DQN: 分为值函数分支和优势函数分支
    # 值函数分支
    value_hidden_layer = Dense(512, activation='relu', kernel_regularizer=reg)(x)
    value_output = Dense(1, activation='linear', name='value_output', kernel_regularizer=reg)(value_hidden_layer)
    # 优势函数分支
    advantage_hidden_layer = Dense(512, activation='relu', kernel_regularizer=reg)(x)
    advantage_output = Dense(19 * 19, activation='linear', kernel_regularizer=reg)(advantage_hidden_layer)

    # 结合值函数和优势函数以获取最终Q值
    def combine_v_and_a(inputs):
        value, advantage = inputs
        q_values = value + (advantage - tf.reduce_mean(advantage, axis=1, keepdims=True))
        return q_values

    q_values = Lambda(combine_v_and_a, name='q_values')([value_output, advantage_output])
...
class ZeroAgent_Update(Agent):
    ...
    # 修改 Sarsa(λ) 更新部分为 Expected Sarsa
    def update_with_expected_sarsa(self, node, epsilon, alpha, gamma):
        while node is not None and node.parent is not None:
            next_node = node.parent
            next_move = next_node.last_move
            if next_move is not None:
                R_s_a = self.calculate_R_s_a(node.state, next_move)
                expected_q = self.get_expected_next_Q(next_node)  # 使用 Expected Sarsa 方式计算下一个状态的 Q 值

                # 使用模型预测的值更新节点的价值
                state_tensor = self.encoder.encode(node.state)
                model_input = np.array([state_tensor])
                predicted_value = self.model.predict(model_input)[0][0]

                # 更新 E 值
                node.E[next_move] = R_s_a + gamma * expected_q - predicted_value
                # 使用 E 值更新模型的值
                predicted_value += alpha * node.E[next_move]
                # 将更新后的值传递给节点
                node.value = predicted_value

            node = next_node

    def get_expected_next_Q(self, node):
        expected_next_Q = 0.0
        legal_moves = node.state.legal_moves()
        for move in legal_moves:
            next_state = node.state.apply_move(move)
            state_tensor = self.encoder.encode(next_state)
            model_input = np.array([state_tensor])
            q_values = self.model.predict(model_input)
            expected_next_Q += q_values[0][0]  # 可以根据具体情况选择要使用的 Q 值
        if legal_moves:
            expected_next_Q /= len(legal_moves)
        return expected_next_Q

    def update_with_dueling_dqn(self, node, gamma):
        # 只在根节点处进行更新
        if node.parent is None:
            ...
            q_values = self.model.predict(model_input)
            value = q_values[0][0]
            advantage = q_values[1][0] - np.mean(q_values[1][0])

            next_max_value = np.max(q_values[0])
            target_value = value + (advantage - np.mean(advantage)) + gamma * next_max_value

            # 将计算出的目标值赋给节点的价值属性
            node.value = target_value

            # 不需要构建目标和预测值，因为直接使用了模型预测的值

            # 使用经验回放训练模型
            experience = {
                'state': model_input,
                'target_q_values': q_values  # 直接使用模型预测的Q值
            }
            self.train_model(experience)

    def train_model(self, experience):
        state_batch = experience['state']
        target_q_values_batch = experience['target_q_values']

        self.model.train_on_batch(state_batch, target_q_values_batch)
