...
class Board:
    def __init__(self, num_rows, num_cols):
        ...
        self._black_captures = 0  # 黑方吃子数量
        self._white_captures = 0  # 白方吃子数量

    # end::init_zobrist[]
    def num_captures(self, player):
        if player == Player.black:
            return self._black_captures
        else:
            return self._white_captures
    ...
    def _remove_string(self, string):
        # 增加对方的吃子数
        capture_count = len(string.stones)
        if string.color == Player.black:
            self._white_captures += capture_count
        else:
            self._black_captures += capture_count
